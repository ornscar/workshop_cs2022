...
author: "Mariana (...) e Rafael Sant'Ana Herzog"
output:
...
Por ser um pacote em R, precisamos, antes de qualquer coisa, instalar o Shiny em seu computador. Para isso, basta que executemos em nosso console o comando .codigo[install.packages("shiny")].
...
Exceto algumas poucas excessões, a grande maioria dos aplicativos em Shiny seguirá essa mesma estrutura.
...
É dentro da UI que decidiremos quais elementos serão mostrados ao usuário e onde eles estarão dispostos. Ela é a “cara” do nosso aplicativo. Neste objeto, não vamos fazer nenhuma manipulação de dados, análises, criação de gráficos ou qualquer outra coisa do tipo. Iremos apenas organizar a interface para que ela possa ser apresentada ao usuário final. 
...
Tudo o que criarmos dentro da UI retornará um código HTML. Dessa forma, estamos, mesmo que sem saber, utilizando funções em R para "programar" em HTML.
...
É importante ressaltar que todas as três listas serão criadas automaticamente quando rodarmos o aplicativo. Os elementos da lista .codigo[input] serão definidos por nós dentro da UI, enquanto os elementos da lista .codigo[output] serão definidos dentro do servidor.
...
As funções que utilizamos para criar inputs seguem um padrão: em geral, elas são da forma <code style = "color: #0A1E3C">&ast;</code>.codigo[Input()], onde o asterisco representa o tipo de input em questão. Para inputs que são botões, o padrão dos nomes das funções costuma ser <code style = "color: #0A1E3C">&ast;</code>.codigo[Button()].

Todas as funções que criam inputs têm o mesmo primeiro argumento: .codigo[inputId]. Esse é o identificador, **que deve ser único** e que será utilizado para acessar cada input dentro do servidor.
...
Vamos construir a UI do aplicativo abaixo. Podemos visualizar esse aplicativo através [deste link](https://rafasantanah.shinyapps.io/meu-primeiro-app/).
...
Vamos, agora, completar a construção do aplicativo que iniciamos anteriormente, escrevendo o código necessário para a criação do gráfico de dispersão dentro do servidor. Novamente, podemos visualizar esse aplicativo através [deste link](https://rafasantanah.shinyapps.io/meu-primeiro-app/).
...
O diagrama de reatividade é um conjunto de dependências que descreve como inputs e outputs estão conectados, sendo responsável por definir a ordem de execução dos códigos que escrevemos dentro do servidor. Esse elemento é responsável por fazer com que uma alteração em um input envie *sinais* para que todos os outputs que dependam desse input sejam recalculados.

No aplicativo criado anteriormente, toda alteração que fazíamos em qualquer um dos inputs resultava automaticamente em uma atualização do gráfico de dispersão que estávamos criando. Isso acontecia porque o Shiny era capaz de reconhecer que o output do gráfico era dependente de todos os três inputs existentes no aplicativo. Um esquema do diagrama de reatividade do aplicativo pode ser visto abaixo.
...
- Os **valores reativos** são as chamadas **fontes de reatividade**. Eles guardam as informações que vêm da UI e disparam sinais de alerta sempre que essas informações mudam. Os valores reativos podem ser lidos por outras expressões reativas, e os exemplos mais comuns deles são aqueles dentro da lista input.

- Os **observers** são o ponto final do diagrama de reatividade. Eles guardam o código de cada output, e recebem os sinais de alerta enviados pelos valores reativos para avaliar a necessidade de se recalcular um output. Os observers podem ler valores reativos e **expressões reativas**, sendo os exemplos mais comuns deles as funções .codigo[render\*()].

- As **expressões reativas** são os chamados **condutores de reatividade**. Ela são capazes de ler valores reativos, fazendo alguma manipulação e devolvendo um resultado que é um valor reativo, e que por isso pode ser utilizado em um observer. Para criar expressões reativas, podemos utilizar as funções .codigo[reactive()] e .codigo[eventReactive()].
...
ui <- fluidPage(
  titlePanel("Criando uma expressão reativa"),
  hr(),
  fluidRow(
    column(
      width = 4,
...
Vamos, agora, utilizar essa função para forçar que a tabela e o gráfico construídos no exemplo anterior só sejam atualizados quando o usuário clicar no botão **"Atualizar a tabela e o gráfico"**, como está na imagem abaixo. O aplicativo completo pode ser acessado por meio deste link.
...
```{r eval=FALSE}
# server
output$tabela <- renderTable({table(mtcars[mtcars$cyl == input$cilindros, ])})
```
...
3. Cada linha pode ser dividida em colunas, para que possamos dispor os elementos horizontalmente;
...
- o .codigo[tabsetPanel()], para criar um aplicativo cuja área principal será dividida em várias abas;

- e a .codigo[navbarPage()], que é semelhante ao .codigo[tabsetPanel()] mas que permite a criação de um menu de navegação superior.
...
Transformando o layout do aplicativo criado anteriormente em um sidebarLayout, teríamos como resultado:
...
Todos os elementos do layout são colocados dentro da função .codigo[fluidPage()]:
...
Criamos um título para o aplicativo através da função .codigo[titlePanel()], como já vimos em outros exemplos:
...
O tabsetPanel não é um layout completo: nós podemos implementá-lo dentro de algum tipo de layout para dividir a área principal de um aplicativo em mais de uma aba. Implementando o tabsetPanel no sidebarlayout do aplicativo criado anteriormente, teríamos o seguinte resultado, que podemos acessar através deste link:
...
Quando queremos criar um layout com um com menu de navegação superior, utilizamos a função .codigo[navbarPage()]. Adaptando o aplicativo criado anteriormente para esse novo tipo de layout, teríamos o seguinte resultado, que pode ser acessado através deste link:
...
Em uma navbarPage, diferente do que acontecia nos outros tipos de layout, tudo aquilo que queremos que esteja na UI deve estar dentro da função .codigo[navbarPage()]. Começando, primeiramente, pela estrutura do menu de navegação superior mostrado no slide anterior; determinamos o título que aparecerá na barra superior através do argumento .codigo[title]:
...
Por meio da função .codigo[tabPanel()], criamos a aba referente à sessão "Sobre":
...
Através da função .codigo[navbarMenu()], criamos uma aba chamada de "Visualizações", na qual estarão agrupadas outras abas dentro de uma caixinha de seleção:
...
Novamente por meio da função .codigo[tabPanel()], criamos as abas "Tabela" e "Gráfico", que estarão disponíveis dentro da caixinha de seleção criada por "Visualizações":
...
A barra de navegação superior resultante desse processo pode ser vista abaixo.

.center[
```{r echo=FALSE, fig.align='center', out.width='95%'}
knitr::include_graphics("img/navbarPage5.png")
```
]
